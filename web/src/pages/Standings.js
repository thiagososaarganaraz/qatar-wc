...
function Standings() {
  ...
  let sorted = []
  standings && standings.map((stand)=> sorted.push((stand.teams).sort((a,b) => {
              ...
              </thead>
                {
                  e.teams.map((team)=>{
                    return(
                      <tbody>
                        <tr key={team.id}>
                          <td><img src={team.flag} style={{width: '40px'}}/> {team.name_en}</td>
                          <td>{team.gf}</td>
                          <td>{team.ga}</td>
                          <td>{team.gd}</td>
                          <td>{team.w}</td>
  ...
  const handleSelection = (e) =>{
    standings && setSelected(standings.filter((stand)=> stand.group === e.target.value))
  }

  return (
    <div className='container' style={{fontFamily : 'Qatar2022'}}>
      <div className='standingsContainer' style={{
        margin: '0 auto'
      }}>
      <nav style={{
        display: 'flex',
        ...
        <select onChange={handleSelection} style={{margin:'auto 1rem', fontFamily:'Qatar2022', fontSize:'18px'}}>
          {!loading ? standings.map((e)=>{
            return(
              <option key={e.group}>{e.group}</option>
            )
          })
          :
        ...
        <button style={{
          fontSize: '20px',
          margin: '0 1rem'
        }} onClick={()=>setSelected(null)}>Mostrar todos</button>
      </nav>
        {!loading ? 
          listStandings()
          :
